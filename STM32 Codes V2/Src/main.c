...
void USER_GPIO_Init( void ) {
  ...
  // Configure PC2 as output (left)
  GPIOC->BSRR = (0x1UL << 9U);
  GPIOC->CRL &= ~(0x3UL << 10U)
		  	 & ~(0x2UL << 8U);
  GPIOC->CRL |= (0x1UL <<8U);

  // Configure PC3 as output (right)
...
int main( void ) {
  ...
  for(;;) {
    ...
    printf("%f,%f,%f,%f\n\r", EngTrModel_Y.EngineSpeed, EngTrModel_Y.VehicleSpeed, EngTrModel_Y.Gear, EngTrModel_U.Throttle);
